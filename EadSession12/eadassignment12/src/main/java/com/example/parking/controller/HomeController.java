...
public class HomeController extends ValidateSession {
	 ...
	 */
	@PostMapping("/login")
	public String dologin(@Validated @ModelAttribute("userObject") UserClass userObject, BindingResult bindingResult,
	 ...
	 */
	@GetMapping("/home/{id}")
	public String friend(@PathVariable("id") int id, Model model, HttpServletResponse response, HttpSession session)
			throws ClassNotFoundException, SQLException {
		if (isSessionValid(session, response)) {
