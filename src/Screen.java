...
public class Screen {
	...
	public Boolean addShapeToScreen(Point point, Shape.shapeType shape,
			List<Integer> dimensionsOfShape) throws Exception {
	...
	public Boolean removeShape(Point point, Shape.shapeType shapeType) throws Exception {
		 ...
		 * Removes the shape.
		 *
		 * @param point the origin of shape.
		...
		if (listOfShapes.size() == 0){
 		...
 		for (int iterator = 0; iterator < listOfShapes.size(); iterator++) {
  			if (listOfShapes.get(iterator).getTypeOfShape() == shapeType && point.getXCoordinate() == listOfShapes.get(iterator).getOrigin().getXCoordinate() && 
  					point.getYCoordinate() == listOfShapes.get(iterator).getOrigin().getYCoordinate()) {
	...
	public ArrayList<Shape> sortOnDistanceFromOrigin() {
		...
		for (int i = 0; i < sortedList.size() - 1; i++) {
			for (int j = 0; j < sortedList.size() - 1; j++) {
				if (sortedList.get(j).getDistanceFromOriginOfScreen() > sortedList
						.get(j + 1).getDistanceFromOriginOfScreen()) {
					Shape temporary = sortedList.get(j + 1);
					sortedList.set(j + 1, sortedList.get(j));
					sortedList.set(j, temporary);
				}
