...
public class ExpressionEvaluation {
	 ...
	 */
	private  int evaluateUnaryExpression(String operand,
			String operator) throws Exception {
		if (!"!".equals(operator)) {
			throw new Exception("Invalid Operator!");
		}
		return !(Integer.parseInt(operand) != 0) ? 1 : 0;
	}
	 ...
	 */
	private  int evaluateBinaryExpression(String operandString1,
	 ...
	 */
	private  void evaluateTopOperator(Stack values, Stack operators)
			throws Exception {
			...
			if ("(".equals(token)) {
				...
				while (!operators.isEmpty()
						...
						&& PRECEDENCE.get(operators.top()) <= PRECEDENCE
								.get(token)) {
					System.out.println( PRECEDENCE.get(operators.top())+""+ PRECEDENCE.get(token));
					evaluateTopOperator(values, operators);
				}
				operators.push(token);
			} else {
