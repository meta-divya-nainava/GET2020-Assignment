...
public class Counsellor {
	...
	private void fetchPrograms(String fileSource) throws Exception {
		...
		Workbook programWorkbook = Workbook.getWorkbook(new File(fileSource));
		...
		for(int i=1;i<programSheetLength;i++){
	...
	private void fetchStudents(String fileSource) throws Exception {
		...
		Workbook studentWorkbook = Workbook.getWorkbook(new File(fileSource));
		...
		for(int i=1;i<studentSheetLength;i++){
			String studentName = studentSheet.getCell(0, i).getContents();
			String[] preference = new String[5];
			
			for(int j=1;j<=5;j++){
				preference[j-1] = studentSheet.getCell(j+1,i).getContents();
			}
	...
	public void allocatePrograms() throws Exception {
		WritableWorkbook allocationWorkbook = Workbook.createWorkbook(new File("files/Allocation1.xls"));
